...
export function generateExtendedColors() {
  ...
  // Try to find the CometChat root element
  let root = document.querySelector('[class="cometchat-root"]') as HTMLElement;
  
  // If not found, try to find any element with CometChat classes
  if (!root) {
    root = document.querySelector('.cometchat-root') as HTMLElement;
  }
  
  // If still not found, try to find the chat container
  if (!root) {
    root = document.querySelector('[style*="width: 70vw"]') as HTMLElement;
  }
  
  // If still not found, use document.body as fallback
  if (!root) {
    root = document.body;
  }
