...
function CometChatHome({ defaultUser, defaultGroup }: CometChatHomeProps) {
  const { chatFeatures, styleFeatures, layoutFeatures } =
    useBuilderSettingContext();
  ...
  const fetchDefaultFreinds = () => {
    console.log("fetchDefaultFreinds")
    const limit = 10,
  ...
  const SideComponentUser = (props: { user: CometChat.User }) => {
    ...
    const actionItemsArray = [
      ...
      },
      {
        ...
        icon: "/icons/comet-chat/assets/delete.svg",
      },
    ];
          ...
          setActionItems([
            {
              ...
              icon: "/icons/comet-chat/assets/block.svg",
            },
            {
              id: 'delete',
              name: getLocalizedString('delete_chat'),
        ...
        CometChat.deleteConversation(UID, 'user')
          .then(() => {
            ...
            CometChatConversationEvents.ccConversationDeleted.next(
              ((selectedItem as Conversation) ||
                (freshChatRef.current as Conversation))!,
            );
            toastTextRef.current = getLocalizedString(
  ...
  return (
    loggedInUser && (
      ...
      >
        ...
        <div
          className={`conversations-wrapper ${!layoutFeatures?.withSideBar ? 'hide-sidebar' : ''}`}
        >
